...
const AdminClientsScreen: React.FC<{
  ...
  onNewBooking: (client: { name: string; phone: string }) => void;
}> = ({ onBack, onChat, onNewBooking }) => {
          ...
          <div key={c.id} className="bg-white dark:bg-surface-dark p-4 rounded-xl border border-gray-200 dark:border-white/5 flex flex-col gap-4 transition-colors">
            <div className="flex items-center gap-4">
              <div className="size-10 rounded-full bg-gray-100 dark:bg-white/5 flex items-center justify-center text-gray-500 font-bold uppercase relative">
                {c.name.charAt(0)}
                {c.active_plan && (
                  <div className="absolute -top-1 -right-1 size-5 bg-amber-500 rounded-full flex items-center justify-center text-black border-2 border-white dark:border-surface-dark shadow-sm">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {c.active_plan && (
                    <span className="text-[9px] font-black bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-md uppercase tracking-tighter">
                      {c.active_plan}
                    </span>
                  )}
                </div>
            ...
            <div className="flex gap-2 border-t border-gray-100 dark:border-white/5 pt-3">
              <button onClick={() => onNewBooking({ name: c.name, phone: c.phone })} className="flex-1 py-2 rounded-lg bg-primary/10 text-primary text-xs font-bold flex items-center justify-center gap-1 hover:bg-primary/20">
                <span className="material-symbols-outlined text-sm">calendar_month</span> Agendar
              </button>
  ...
  <div className="relative flex h-full min-h-screen w-full flex-col overflow-x-hidden pb-24 bg-gradient-to-b from-primary/20 to-white dark:bg-background-dark transition-colors">
    ...
    <main className="flex-1 flex flex-col px-4 pt-4">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <button onClick={onAssinatura} className="relative group flex flex-col items-start justify-end p-4 h-40 w-full rounded-2xl overflow-hidden shadow-lg border border-gray-200 dark:border-white/5 active:scale-[0.98] transition-all col-span-2">
          <div className="absolute inset-0 z-0">
            <img alt="Clube de Assinatura" className="h-full w-full object-cover" src="https://images.unsplash.com/photo-1503951914875-452162b0f3f1?q=80&w=2070&auto=format&fit=crop" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent"></div>
          </div>
          <div className="relative z-10 flex flex-col items-start gap-1">
            <div className="mb-1 rounded-full bg-amber-500 p-2 text-white">
              <span className="material-symbols-outlined text-[20px]">card_membership</span>
            </div>
            <span className="text-left text-sm font-bold leading-tight text-white">Clube de Assinatura</span>
          </div>
        </button>
      </div>
          ...
          <div key={msg.id} className={`flex ${msg.sender === currentUserRole ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] p-3 rounded-2xl text-sm ${msg.sender === currentUserRole
              ...
              <div className={`text-[10px] mt-1 opacity-50 ${msg.sender === currentUserRole ? 'text-right' : 'text-left'}`}>
                {msg.timestamp.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })} - {msg.timestamp.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
              </div>
            </div>
          </div>
        ))}
...
const AdminDashboard: React.FC<{
  ...
  unreadCount: number;
}> = ({ appointments, showPastHistory, setShowPastHistory, onLogout, onOpenChat, onManageServices, onBlockSchedule, onSettings, onWeeklySchedule, onFinance, onTV, onSubscriptions, onManagePlans, onRefresh, onClients, onNewBooking, setAppointments, unreadCount }) => {
      ...
      <main className="p-4 pb-24 max-w-md mx-auto w-full">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <button
            ...
            className="relative group flex flex-col p-4 rounded-3xl bg-primary border border-primary/30 hover:brightness-110 active:scale-[0.98] transition-all overflow-hidden shadow-lg h-32 justify-between col-span-2"
          >
            <div className="size-10 rounded-xl bg-white/20 flex items-center justify-center text-white shadow-lg">
              <span className="material-symbols-outlined filled">add_circle</span>
            </div>
            <div className="text-left">
              <h3 className="font-bold text-white text-lg">Novo Agendamento</h3>
              <p className="text-white/70 text-[10px] font-bold uppercase tracking-widest">Agendar para Cliente</p>
            </div>
          </button>
...
const App: React.FC = () => {
  ...
  const handleFinishBooking = async () => {
    ...
    setTimeout(() => {
      ...
      if (currentUserRole === 'BARBER') setView('ADMIN_DASHBOARD');
      else setView('MY_APPOINTMENTS');
    }, 3000);
  };
    ...
    switch (view) {
      ...
      case 'CHAT':
        return <ChatScreen
          ...
          customerIdentity={currentUserRole === 'BARBER' && selectedChatClient 
            ? { name: selectedChatClient.name, phone: '' } 
