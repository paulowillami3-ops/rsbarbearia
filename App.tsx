...
const AdminTVScreen: React.FC<{ appointments: Appointment[]; onBack: () => void; onRefresh: () => void }> = ({ appointments, onBack, onRefresh }) => {
  ...
  return (
    <div className="bg-slate-900 min-h-screen flex flex-col p-8 text-white overflow-hidden">
      <header className="flex justify-between items-center mb-10 border-b border-white/10 pb-6">
        <div className="flex items-center gap-6">
          <div className="h-20 w-20 bg-white rounded-full p-2 flex items-center justify-center">
            <img src="/logo.png" className="h-full w-full object-contain" />
          </div>
          <div>
            <h1 className="text-4xl font-black tracking-tight text-white shadow-black drop-shadow-lg">Agendamentos do Dia</h1>
            <p className="text-xl text-gray-400 mt-1 uppercase tracking-widest font-bold">{currentTime.toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
          </div>
        </div>
        <div className="text-right">
          <div className="text-7xl font-black font-mono tracking-widest text-primary drop-shadow-[0_0_15px_rgba(212,17,50,0.5)]">
            {currentTime.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
          </div>
          <button onClick={onBack} className="mt-4 px-6 py-2 rounded-full bg-white/5 hover:bg-white/10 text-sm font-bold uppercase tracking-wider text-gray-400 hover:text-white transition-all border border-white/5">
            Sair do Modo TV
          </button>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto no-scrollbar pb-20">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {activeApps.length === 0 ? (
            <div className="col-span-full flex flex-col items-center justify-center p-20 opacity-30 mt-20">
              <span className="material-symbols-outlined text-[150px] mb-8">event_busy</span>
