...
const AdminClientsScreen: React.FC<{ onBack: () => void; onChat: (id: string, name: string) => void }> = ({ onBack, onChat }) => {
  ...
  const fetchClients = async () => {
    const { data, error } = await supabase
      .from('clients')
      .select('id, name, phone, subscriptions:user_subscriptions(plan:subscription_plans(name), status)')
...
const SelectDateTimeScreen: React.FC<{
  ...
  useEffect(() => {
    const initData = async () => {
      // 14-day window for availability
      const todayStr = format(new Date(), 'yyyy-MM-dd');
      const maxDateStr = format(addDays(new Date(), 14), 'yyyy-MM-dd');

      // Fetch Work Hours
      const { data: wh } = await supabase.from('work_hours').select('id, day_of_week, is_open, start_time_1, end_time_1, start_time_2, end_time_2');
      if (wh) setWorkHours(wh);

      // Fetch Blocks for the current window
      ...
      // Fetch Appointments for the current window with Service Durations
      const { data: apps } = await supabase
        .from('appointments')
        .select('id, appointment_date, appointment_time, status, services:appointment_services(service:services(duration))')
...
const AdminFinanceScreen: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const loadData = async () => {
    ...
    // 1. Fetch Expenses (filtered by date)
    ...
    // 2. Fetch Appointments (filtered by date)
    const { data: appData } = await supabase
      .from('appointments')
      .select('id, appointment_date, total_price, status, client_id, services:appointment_services(service:services(name)), clients(name)')
      .eq('status', 'COMPLETED')
    ...
    // 3. Fetch Approved Subscriptions (filtered by date)
    // Subscriptions might need special handling if date is based on approved_at or created_at
    const { data: subData } = await supabase
      .from('user_subscriptions')
      .select('id, created_at, approved_at, status, plan:subscription_plans(price)')
      .eq('status', 'APPROVED')
      // Note: We'll filter approved_at in SQL if possible, or stay with this for now
  ...
  useEffect(() => { loadData(); }, [dateRange]); // RELOAD when dateRange changes!
  ...
  const fetchAppointments = useCallback(async () => {
    ...
    if (data && data.length > 0) {
      ...
      const [psDataFetch, scDataFetch, monthAppsFetch] = await Promise.all([
        ...
        supabase.from('service_components').select('parent_service_id, component_service_id'),
