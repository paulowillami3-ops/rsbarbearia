...
app.post('/api/appointments', (req, res) => {
    ...
    db.get("SELECT id FROM appointments WHERE appointment_date = ? AND appointment_time = ? AND status != 'CANCELLED'", [date, time], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
        if (row) {
            return res.status(400).json({ error: "Este horário já foi reservado por outro cliente." });
        }

        // Check for blocked slot
        db.get("SELECT id FROM blocked_slots WHERE date = ? AND time = ?", [date, time], (err, row) => {
            if (err) return res.status(500).json({ error: err.message });
            if (row) {
                return res.status(400).json({ error: "Este horário está bloqueado." });
            }
